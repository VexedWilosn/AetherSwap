...
def _load_config() -> dict[str, Any]:
    ...
    enabled_default = bool(price_cfg.get("enabled", False))
...
def _select_due_items(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    with SessionLocal() as db:
        ...
        last_price_rows = (
            ...
            .filter(
                ...
                MarketPrice.data_source == "steamdt_openapi",
            )
    ...
    for row in items:
        ...
        times = platform_times_by_item.get(item_id, {})
        missing = len(tracked - set(times.keys()))
    ...
    due.sort(
        key=lambda x: (
            -int(x["crawl_priority"]),
            -int(x["missing_platforms"]),
            x["last_updated_at"] or datetime.fromtimestamp(0),
            x["item_id"],
        )
    )
    return due
...
def run_once() -> dict[str, Any]:
    ...
    pool_names = [str(row["market_hash_name"]) for row in pool_due_items]
    discovery_names = [str(row["market_hash_name"]) for row in discovery_due_items]
    selected_names = pool_names[:pool_budget_items] + discovery_names[:discovery_budget_items]
    if not selected_names:
        selected_names = pool_names[:]
