...
| Live canary operator gate | Completed in this pass | `/api/trade/live_canary/status`, `/api/trade/live_canary/precheck`, smoke registry snapshot, single-action gate decision |

## Remaining Gaps

| Gap | Priority | Notes |
|---|---:|---|
| Real small-value platform smoke loops | P1/manual gate | Operator gate and runbook now exist; the actual low-capital live test remains intentionally manual and must use `live_canary` with `limit=1`. |
...
## Latest Implementation Note 3

The latest pass made the minimum-capital live canary path operator-executable without enabling uncontrolled live trading:

- Added `LiveCanarySmokeRegistry.snapshot()` and `inspect_live_canary_run()` for machine-readable canary gate inspection.
- Added `GET /api/trade/live_canary/status` with config, worker status, action summary, smoke snapshot, and next-action gate result.
- Added `POST /api/trade/live_canary/precheck` to return whether `POST /api/trade/platform_actions/run_once` is safe with `{"safe_mode": false, "limit": 1}`.
- Added `channel` filtering to `GET /api/trade/platform_actions` for isolating `live_canary` ledger rows during manual verification.
- Added `analysis/live_canary_minimum_capital_runbook_20260510.md` with the T0-T4 minimum-capital test sequence.
