            ...
            for opportunity in opportunities:
                ...
                if platform == "buff":
                    ...
                    if safe_mode:
                        ...
                        try:
                            result = buff_buyer.create_buy_order(goods_id=goods_id, price=float(opportunity.buy_price), num=max(1, int(opportunity.quantity or 1)), game=DEFAULT_GAME)
                ...
                if opportunity.decision_id is not None:
                    mark_decision_done(session, opportunity.decision_id, "success" if isinstance(result, dict) and result.get("success") else "failed", result)
                else:
                    mark_opportunity_verifying(session, opportunity.id)
                session.commit()
