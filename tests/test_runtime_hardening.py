...
from DataEngine.database import ActionDecision, ArbitrageOpportunity, Base, ItemBase, MarketPrice
...
from DataEngine.trade_executor import OpportunityView, SUPPORTED_BUY_PLATFORMS, _request_buff_seller_offer, should_skip_jit_refresh
...
def test_priority_scheduler_promotes_cashout_below_steam_nominal_price():
    ...
    decision = compute_priority_decision(
        ...
        [],
        config={
            "pipeline": {"steam_balance_cost_ratio": 0.55},
    ...
    assert "cashout_55_discount_profit" in decision.reason
    assert "spread=0.4182" in decision.reason
...
def test_priority_scheduler_cashout_discount_tiers_target_priorities():
    ...
    p55 = compute_priority_decision(
        SimpleNamespace(id=1, crawl_priority=1),
        base_prices,
        [],
        config={**cfg, "pipeline": {"steam_balance_cost_ratio": 0.55}},
    )
    assert p55.priority == PRIORITY_HIGH_FREQ
    assert "cashout_55_discount_profit" in p55.reason

    p63 = compute_priority_decision(
        SimpleNamespace(id=1, crawl_priority=3),
        base_prices,
        [],
        config={**cfg, "pipeline": {"steam_balance_cost_ratio": 0.63}},
    )
    assert p63.priority == PRIORITY_STEAMDT_CANDIDATE
    assert "cashout_63_discount_profit" in p63.reason

    p70 = compute_priority_decision(
        SimpleNamespace(id=1, crawl_priority=3),
        base_prices,
        [],
        config={**cfg, "pipeline": {"steam_balance_cost_ratio": 0.70}},
    )
    assert p70.priority == PRIORITY_LOW_FREQ
...
def test_recalculate_priorities_uses_full_app_config_for_cashout_ratio(monkeypatch):
    ...
    with TestingSessionLocal() as session:
        ...
        assert "spread=0.4182" in item.priority_reason
...
def test_arbitrage_scanner_ignores_baseline_cash_platform_prices(monkeypatch):
    captured = []
    ...
    monkeypatch.setattr(
        ...
        lambda session: {
            1: {
                "steam": SimpleNamespace(sell_min=0.47, buy_max=0.0, data_source="steamdt_openapi"),
                "buff": SimpleNamespace(sell_min=0.01, buy_max=0.0, data_source="baseline"),
                "uuyp": SimpleNamespace(sell_min=0.02, buy_max=0.0, data_source="baseline"),
            }
        },
    )
    monkeypatch.setattr(scanner, "_close_non_decision_opportunities", lambda session, latest_prices: 0)
    monkeypatch.setattr(scanner, "_bulk_upsert", lambda session, opportunities: captured.extend(opportunities))
...
def test_arbitrage_scanner_closes_existing_baseline_opportunities(monkeypatch):
    engine = create_engine("sqlite:///:memory:", future=True)
    Base.metadata.create_all(engine)
    TestingSessionLocal = sessionmaker(bind=engine, autoflush=False, autocommit=False, future=True)
    monkeypatch.setattr(
        "DataEngine.arbitrage_engine.load_pipeline_config",
        lambda: {"pipeline": {"steam_balance_cost_ratio": 0.7, "max_discount": 0.8}},
    )

    with TestingSessionLocal() as session:
        session.add(ItemBase(id=1, market_hash_name="Test Item"))
        session.add_all(
            [
    ...
    scanner = ArbitrageScanner(session_factory=TestingSessionLocal)
    opportunities = scanner.scan_opportunities()

    assert opportunities == []
    with TestingSessionLocal() as session:
        assert session.get(ArbitrageOpportunity, 1).status == "closed"
...
def test_action_policy_direct_buy_first_overrides_buy_order_score():
    opportunity = SimpleNamespace(
        id=44,
        item_id=42,
        buy_platform="buff",
    ...
    assert decision.action == ACTION_DIRECT_BUY
    assert "direct_buy_first" in decision.reason


def test_action_policy_blocks_low_price_exposure_quota(tmp_path, monkeypatch):
    engine = create_engine(f"sqlite:///{tmp_path / 'action_policy_exposure_guard.db'}")
    Base.metadata.create_all(engine, tables=[ActionDecision.__table__])
    SQLModel.metadata.create_all(engine, tables=[Purchase.__table__, PlatformAction.__table__])
    ...
    item = SimpleNamespace(id=42, market_hash_name="Cheap Skin")
    prices = {"buff": SimpleNamespace(volume=100), "steam": SimpleNamespace(volume=100)}

    with SessionLocal() as session:
        session.add(Purchase(goods_id=42, name="Cheap Skin", price=0.02))
        session.commit()
        decision = compute_action_decision(
            opportunity,
            item,
            prices,
            config={
                "action_policy": {"min_24h_volume": 0, "direct_buy_min_profit_rate": 0.01},
                "low_price_exposure_guard": {"enabled": True, "rule": "0-1-0.05", "block_execution": True},
            },
            db=session,
        )

    assert decision.action == ACTION_BLOCKED
    assert decision.reason == "low_price_exposure_quota"


def test_trade_executor_requests_buff_seller_offer_for_all_direct_buy_orders():
    calls = []

    class FakeBuffBuyer:
        def ask_seller_to_send(self, order_ids, game="csgo"):
            calls.append((order_ids, game))
            return {"success": True, "data": {"requested": list(order_ids)}}

    result = {
        "success": True,
        "order_id": "bill-1",
        "data": {
            "order_ids": ["bill-1", "bill-2"],
            "bill_order_ids": ["bill-2", "bill-3"],
        },
    }

    _request_buff_seller_offer(FakeBuffBuyer(), result, game="csgo")

    assert calls == [(["bill-1", "bill-2", "bill-3"], "csgo")]
    assert result["seller_offer_request"]["success"] is True
