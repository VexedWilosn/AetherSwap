...
def test_radar_snapshot_treats_steamdt_openapi_volume_as_depth(db_session):
    ...
    assert round(snap.steam_to_cash_profit_rate, 2) == round(((4.0 * 0.975 - 10.0 * 0.70) / (10.0 * 0.70)) * 100.0, 2)
...
def test_radar_api_applies_exposure_guard_before_pagination_and_total(monkeypatch):
    import app.api as api
    ...
    monkeypatch.setattr(
        api,
        "load_app_config",
        lambda: {"low_price_exposure_guard": {"enabled": True, "rule": "0-1-0.05", "hide_signals": True}},
    )

    with TestingSessionLocal() as session:
        for idx, profit in [(1, 100.0), (2, 90.0), (3, 80.0)]:
            session.add(ItemBase(id=940000 + idx, market_hash_name=f"Radar Guard {idx}", crawl_priority=3))
            session.add(
                RadarSnapshot(
                    item_id=940000 + idx,
                    item_name=f"Radar Guard {idx}",
                    market_hash_name=f"Radar Guard {idx}",
                    crawl_priority=3,
                    best_platform="buff",
                    best_platform_price=0.03,
                    steam_sell_min=0.10,
                    best_profit_rate=profit,
                    cash_to_steam_profit_rate=profit,
                    cash_to_steam_platform="buff",
                    cash_to_steam_price=0.03,
                    steam_to_cash_profit_rate=-10.0,
                    steam_to_cash_platform="buff",
                    steam_to_cash_price=0.01,
                    platform_payload_json="{}",
                    snapshot_updated_at=datetime.now(),
                )
            )
        session.add(Purchase(goods_id=940001, name="Radar Guard 1", price=0.03))
        session.add(Purchase(goods_id=940002, name="Radar Guard 2", price=0.03))
        session.commit()

    client = TestClient(api.app)
    response = client.get("/api/market/radar", params={"limit": 2, "offset": 0, "sort_by": "profit_rate", "sort_dir": "desc"})

    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
    assert [row["market_hash_name"] for row in data["items"]] == ["Radar Guard 3"]
    engine.dispose()


def test_radar_legacy_applies_exposure_guard_before_pagination_and_total(monkeypatch):
    import app.api as api
    ...
    with TestingSessionLocal() as session:
        for idx, sell_min in [(1, 0.01), (2, 0.02), (3, 0.03)]:
            session.add(ItemBase(id=950000 + idx, market_hash_name=f"Legacy Guard {idx}", crawl_priority=3))
            session.add(
                MarketPrice(
                    item_id=950000 + idx,
                    platform_name="steam",
        ...
        session.add(Purchase(goods_id=950001, name="Legacy Guard 1", price=0.01))
        session.add(Purchase(goods_id=950002, name="Legacy Guard 2", price=0.02))
        session.commit()

    client = TestClient(api.app)
    response = client.get("/api/market/radar_legacy", params={"limit": 2, "offset": 0, "sort_by": "profit_rate", "sort_dir": "desc"})

    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
    assert [row["market_hash_name"] for row in data["items"]] == ["Legacy Guard 3"]
    engine.dispose()
