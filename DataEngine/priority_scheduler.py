...
from DataEngine.profit_model import best_profit_signal_from_prices, get_after_tax_price, safe_float
...
DEFAULT_SCHEDULER_CONFIG = {
    ...
    "cashout_excellent_balance_cost_ratio": 0.75,
    "cashout_good_balance_cost_ratio": 0.70,
    "cashout_pass_balance_cost_ratio": 0.65,
...
def _cashout_tier_signal(prices: list[MarketPrice], cfg: dict[str, Any]) -> tuple[str | None, float, float]:
    steam_sell = 0.0
    cashout_revenue = 0.0
    for price in prices:
    ...
    thresholds = [
        ("cashout_75_discount_profit", float(cfg.get("cashout_excellent_balance_cost_ratio", 0.75) or 0.75), float(cfg.get("cashout_excellent_score_floor", 75) or 75)),
        ("cashout_70_discount_profit", float(cfg.get("cashout_good_balance_cost_ratio", 0.70) or 0.70), float(cfg.get("cashout_good_score_floor", 60) or 60)),
        ("cashout_65_discount_profit", float(cfg.get("cashout_pass_balance_cost_ratio", 0.65) or 0.65), float(cfg.get("cashout_pass_score_floor", 50) or 50)),
    ]
    for label, ratio, score_floor in thresholds:
        cost = steam_sell * ratio
        if cost > 0 and cashout_revenue > cost:
            return label, score_floor, (cashout_revenue - cost) / cost
    return None, 0.0, 0.0


def _cashout_tier_priority(tier_label: str | None) -> int | None:
    if tier_label == "cashout_75_discount_profit":
        return PRIORITY_HIGH_FREQ
    if tier_label == "cashout_70_discount_profit":
        return PRIORITY_STEAMDT_CANDIDATE
    if tier_label == "cashout_65_discount_profit":
...
def _latest_price_signal(prices: list[MarketPrice], config: dict[str, Any] | None = None, scheduler_cfg: dict[str, Any] | None = None) -> tuple[int, float, float, bool, str | None]:
    ...
    tier_label, tier_score_floor, tier_spread = _cashout_tier_signal(prices, scheduler_cfg or DEFAULT_SCHEDULER_CONFIG)
...
def compute_priority_decision(
    ...
    *,
    ...
    if cashout_tier:
        _, score_floor, _ = _cashout_tier_signal(prices, cfg)
