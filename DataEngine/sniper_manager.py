...
try:
    from DataEngine.database import ArbitrageOpportunity, ItemBase, MarketPrice, SessionLocal, engine, upsert_market_price_if_fresh
...
UUYP_MARKET_SALE_URL = "https://api.youpin898.com/api/homepage/pc/goods/market/queryOnSaleCommodityList"
...
class UuypSniper(BaseSniper):
    ...
    async def fetch_price(self, template_id: str) -> Optional[dict[str, Any]]:
        url = UUYP_MARKET_SALE_URL
        try:
            from DataEngine.uuyp_public_monitor import _build_uuyp_request_context
        except ModuleNotFoundError:
            from uuyp_public_monitor import _build_uuyp_request_context

        headers, cookies = _build_uuyp_request_context(str(template_id))
        request_payload = {
        ...
        payload = await self._safe_post_json(
            url,
            request_payload,
            headers=headers,
            cookies=cookies,
    ...
    def _normalize(self, payload: dict[str, Any]) -> Optional[dict[str, Any]]:
        ...
        if code not in (0, "0", 200, "200") and payload.get("success") is not True:
            return None
        ...
        for row in rows if isinstance(rows, list) else []:
            ...
            price = self._to_float(
                ...
                or row.get("Price")
                or row.get("SellPrice")
                or row.get("commodityPrice")
            )
            if price > 0:
                prices.append(price)
        if not prices:
            return None
        sell_volume = self._to_int(
