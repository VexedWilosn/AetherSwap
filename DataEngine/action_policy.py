    ...
    *,
    ...
    try:
        ...
        if not exposure_decision.allowed:
            return _decision_for_blocked(
                opportunity,
                exposure_decision.reason,
                exposure_decision.message or f"current={exposure_decision.current_quantity} max={exposure_decision.max_quantity}",
            )
    except Exception as exc:
        logger.warning("low price exposure guard unavailable | item_id=%s err=%s", item.id, exc)
    ...
    if cfg.get("allow_direct_buy", True) and expected_rate >= direct_min_rate:
        ...
        direct_score = _risk_adjusted_score(
        ...
        if bool(cfg.get("direct_buy_first", True)):
            return ActionPolicyDecision(
                opportunity_id=opportunity.id,
                item_id=opportunity.item_id,
                action=ACTION_DIRECT_BUY,
                ...
                requires_jit=requires_jit,
                status="open",
                reason=f"direct_buy_first score={direct_score} rate={expected_rate:.4f} volume={volume}",
...
def generate_action_decisions(config: dict[str, Any] | None = None) -> int:
    ...
    cfg = action_policy_config(app_config)
    if not cfg.get("enabled", True):
