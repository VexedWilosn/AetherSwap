...
class FakeBuffClient:
    def __init__(self):
        self.calls = []
        self.direct_buy_calls = []
        self.wait_pay_calls = []
        self.cancel_calls = []
        self.reprice_calls = []
        self.ask_seller_calls = []

    def create_buy_order(self, **kwargs):
        self.calls.append(kwargs)
        return {"success": True, "msg": "ok", "order_id": "buff-order-1"}

    def direct_buy(self, **kwargs):
        self.direct_buy_calls.append(kwargs)
        return {
            "success": True,
            "msg": "ok",
            "order_id": "buff-direct-1",
            "sell_order_id": kwargs.get("sell_order_id") or "sell-order-1",
            "order_status": "pending",
            "filled_quantity": 1,
            "remaining_quantity": 0,
            "filled_amount_cny": kwargs.get("price"),
            "data": {"bill_order_ids": ["buff-direct-1"]},
        }

    def ask_seller_to_send(self, bill_order_id_or_ids, game="csgo"):
        self.ask_seller_calls.append((bill_order_id_or_ids, game))
        return {
            "success": True,
            "msg": "BUFF seller offer requested",
            "data": {"requested": [str(bill_order_id_or_ids)], "order_status": "wait_buyer_confirm"},
        }
...
class FakeBuffStatusClient(FakeBuffClient):
    def __init__(self):
        ...
        self.status_calls = []
        self.ask_seller_calls = []
        self.status_result = {"success": True, "msg": "ok", "data": {"id": "buff-order-1", "state": "success"}}
...
class FakeUuypStatusClient:
    def __init__(self):
        ...
        self.status_result = {"success": True, "msg": "ok", "data": {"OrderNo": "uuyp-order-1", "orderStatus": "pending"}}
...
class FakeUuypDirectClient:
    def __init__(self):
        self.last_select_listing_error = None
        self.select_calls = []
        self.buy_calls = []

    def select_best_listing(self, template_id, **kwargs):
        self.select_calls.append((template_id, kwargs))
        return None

    def buy_listing(self, **kwargs):
        self.buy_calls.append(kwargs)
        return {"success": True, "msg": "ok", "order_id": "uuyp-direct-1"}
...
class FakeEcoClient:
    def __init__(self):
        ...
        self.reprice_calls = []
        self.off_shelf_calls = []
        self.status_result = {"success": True, "msg": "ok", "data": {"OrderStatus": "completed", "OrderNo": "eco-order-1"}}
...
def test_buff_adapter_submits_direct_buy_with_payload_mapping():
    ...
    action = PlatformAction(
        ...
        target_price=100,
        quantity=2,
        request_payload='{"goods_id": 123, "sell_order_id": "sell-order-1"}',
    )

    result = adapter.submit(action)

    assert result.success is True
    assert result.platform_order_id == "buff-direct-1"
    assert client.direct_buy_calls == [
        {
            "goods_id": 123,
            "price": 100.0,
            "num": 2,
    ...
    assert client.ask_seller_calls == [(["buff-direct-1"], "csgo")]
    assert result.response_payload["seller_offer_request"]["success"] is True
    assert result.request_payload["goods_id"] == 123
    assert result.request_payload["sell_order_id"] == "sell-order-1"
    assert client.calls == []
    assert factory.purposes == [("buff", "direct_buy")]


def test_buff_adapter_keeps_purchase_order_on_buy_order_endpoint():
    client = FakeBuffClient()
    factory = FakeFactory(client)
    adapter = PlatformClientAdapter("buff", factory=factory)
    action = PlatformAction(
    ...
    assert client.direct_buy_calls == []
    assert factory.purposes == [("buff", "purchase_order")]
...
def test_eco_adapter_recovers_merchant_no_actions_with_merchant_query():
    client = FakeEcoClient()
    client.status_result = {
        "success": True,
        "msg": "ok",
        "data": {
            "OrderNum": "eco-order-1",
            "MerchantNo": "ASECO1778534001949",
            "OrderState": "交易保护",
            "OrderStateCode": 30,
            "TradeOfferId": "offer-eco-1",
            "AssetId": "asset-eco-1",
        },
    }
    factory = FakeFactory(client)
    ...
    action = PlatformAction(
        ...
        item_id=1,
        market_hash_name="M4A1-S | Printstream (Field-Tested)",
        platform_order_id="ASECO1778534001949",
    )
    ...
    assert result.category == "order_completed"
    assert result.platform_order_id == "eco-order-1"
    assert result.trade_offer_id == "offer-eco-1"
    assert result.assetid == "asset-eco-1"
    assert client.status_calls == [{"game_id": "730", "merchant_nos": ["ASECO1778534001949"]}]
...
def test_buff_adapter_requests_seller_offer_when_order_waits_for_seller_send():
    client = FakeBuffStatusClient()
    client.status_result = {
        "success": True,
        "msg": "ok",
        "data": {"id": "buff-order-1", "state": "wait_seller_send_offer"},
    }
    factory = FakeFactory(client)
    adapter = PlatformClientAdapter("buff", factory=factory)
    action = PlatformAction(
    ...
    assert result.category == "order_pending"
    assert result.platform_order_id == "buff-order-1"
    assert client.ask_seller_calls == [("buff-order-1", "csgo")]
    assert result.response_payload["seller_offer_request"]["success"] is True


def test_buff_adapter_moves_to_trade_offer_when_status_contains_offer_id():
    client = FakeBuffStatusClient()
    client.status_result = {
        "success": True,
        "msg": "ok",
        "data": {
            "id": "buff-order-1",
            "state": "wait_buyer_confirm",
            "tradeOfferId": "offer-buff-1",
        },
    }
    factory = FakeFactory(client)
    adapter = PlatformClientAdapter("buff", factory=factory)
    ...
    assert result.category == "order_pending"
    assert result.platform_order_id == "buff-order-1"
    assert result.trade_offer_id == "offer-buff-1"
    assert client.ask_seller_calls == []
...
def test_uuyp_adapter_extracts_trade_offer_from_pending_status():
    client = FakeUuypStatusClient()
    client.status_result = {
        "success": True,
        "msg": "ok",
        "data": {
            "OrderNo": "uuyp-order-1",
            "orderStatus": "wait_buyer_confirm",
            "tradeOfferId": "offer-uuyp-1",
            "AssetId": "asset-uuyp-1",
        },
    }
    factory = FakeFactory(client)
    adapter = PlatformClientAdapter("uuyp", factory=factory)
    ...
    assert result.category == "order_pending"
    assert result.platform_order_id == "uuyp-order-1"
    assert result.trade_offer_id == "offer-uuyp-1"
    assert result.assetid == "asset-uuyp-1"


def test_uuyp_direct_buy_is_reported_as_unsupported_not_not_found():
    client = FakeUuypDirectClient()
    client.last_select_listing_error = {"code": 85100, "status_code": 429, "msg": "当前app版本过低，请前往更新"}
    factory = FakeFactory(client)
    adapter = PlatformClientAdapter("uuyp", factory=factory)
    action = PlatformAction(
        id=18,
        ...
        platform="uuyp",
        item_id=1,
        market_hash_name="P250 | Copper Oxide (Field-Tested)",
        target_price=0.02,
        quantity=1,
        request_payload='{"template_id": "110797"}',
    )
    ...
    assert "direct_buy is not supported" in result.message
    assert result.response_payload["reason"] == "direct_buy_unsupported"
