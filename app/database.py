...
class Purchase(SQLModel, table=True):
    __table_args__ = (
        Index("ix_purchase_source_action_fill", "source_action_id", "source_fill_index", unique=True),
        Index("ix_purchase_source_order", "source_platform", "source_order_id"),
        Index("ix_purchase_source_trade_offer_id", "source_trade_offer_id"),
    )
    ...
    source_action_id: Optional[int] = Field(default=None)
    source_order_id: Optional[str] = Field(default=None, max_length=128)
    source_trade_offer_id: Optional[str] = Field(default=None, max_length=128)
    source_fill_index: Optional[int] = Field(default=None)
...
class PlatformAction(SQLModel, table=True):
    ...
    archived_at: Optional[float] = Field(default=None, index=True)
    archived_reason: Optional[str] = None
    archived_by: Optional[str] = Field(default=None, max_length=80)
...
def _ensure_platform_action_archive_columns() -> None:
    columns = {
        "archived_at": "FLOAT",
    ...
    with engine.connect() as conn:
        try:
            ...
            conn.execute(
                sa_text(
                    "CREATE INDEX IF NOT EXISTS ix_platform_action_archived_at "
                    "ON platform_action (archived_at)"
                )
            )
            conn.commit()
        except Exception:
...
def _ensure_purchase_source_columns() -> None:
    columns = {
        "source_platform": "VARCHAR(50)",
        "source_action_id": "INTEGER",
        "source_order_id": "VARCHAR(128)",
        "source_trade_offer_id": "VARCHAR(128)",
        "source_fill_index": "INTEGER",
    }
    with engine.connect() as conn:
        try:
            inspector = inspect(conn)
            if "purchase" not in inspector.get_table_names():
                return
            existing = {col["name"] for col in inspector.get_columns("purchase")}
            for name, ddl in columns.items():
                if name not in existing:
                    conn.execute(sa_text(f"ALTER TABLE purchase ADD COLUMN {name} {ddl}"))
            conn.execute(
                sa_text(
                    "CREATE UNIQUE INDEX IF NOT EXISTS ix_purchase_source_action_fill "
                    "ON purchase (source_action_id, source_fill_index)"
                )
            )
            conn.execute(
                sa_text(
                    "CREATE INDEX IF NOT EXISTS ix_purchase_source_order "
                    "ON purchase (source_platform, source_order_id)"
                )
            )
            conn.execute(
                sa_text(
                    "CREATE INDEX IF NOT EXISTS ix_purchase_source_trade_offer_id "
                    "ON purchase (source_trade_offer_id)"
                )
            )
            conn.commit()
        except Exception:
...
def init_db() -> None:
    ...
    _ensure_platform_action_archive_columns()
    _ensure_purchase_source_columns()
...
def _purchase_from_dict(d: dict) -> Purchase:
    return Purchase(
        ...
        source_platform=str(d["source_platform"]) if d.get("source_platform") is not None else None,
        source_action_id=int(d["source_action_id"]) if d.get("source_action_id") is not None else None,
        source_order_id=str(d["source_order_id"]) if d.get("source_order_id") is not None else None,
        source_trade_offer_id=str(d["source_trade_offer_id"]) if d.get("source_trade_offer_id") is not None else None,
        source_fill_index=int(d["source_fill_index"]) if d.get("source_fill_index") is not None else None,
    )
...
def _purchase_to_dict(p: Purchase) -> dict:
    ...
    if p.source_platform is not None:
        d["source_platform"] = p.source_platform
    if p.source_action_id is not None:
        d["source_action_id"] = p.source_action_id
    if p.source_order_id is not None:
        d["source_order_id"] = p.source_order_id
    if p.source_trade_offer_id is not None:
        d["source_trade_offer_id"] = p.source_trade_offer_id
    if p.source_fill_index is not None:
        d["source_fill_index"] = p.source_fill_index
    return d
...
_PURCHASE_UPDATABLE = frozenset({
    ...
    "source_platform", "source_action_id", "source_order_id",
    "source_trade_offer_id", "source_fill_index",
})
