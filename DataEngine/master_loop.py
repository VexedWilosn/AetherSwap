...
def _auto_trading_enabled(config: dict) -> bool:
    trading = config.get("automation_modules") if isinstance(config.get("automation_modules"), dict) else {}
    if "auto_trading_enabled" in trading:
        raw = trading.get("auto_trading_enabled")
        return raw if isinstance(raw, bool) else str(raw).strip().lower() in {"1", "true", "yes", "on", "enabled"}
    raw = os.getenv("AETHERSWAP_AUTO_TRADING_ENABLED")
    if raw is not None:
...
def main() -> None:
    ...
    while True:
        try:
            ...
            if _auto_trading_enabled(app_config):
                _run_script("action_policy.py")
                _run_script("trade_executor.py")
            else:
                logger.info("auto trading skipped | reason=automation_modules.auto_trading_enabled=false")
